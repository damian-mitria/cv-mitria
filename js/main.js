...
window.onload = function () {
    ...
    var button = document.getElementById("hola");
    var saviorButton = document.getElementById("chau");
    ...
    button.addEventListener("click", function () {
        ...
        // Alert
        alert("Decia que no lo presione 😏, ha roto mi página!... Contráteme!");
        // Hide the button
        button.style.display = "none";
        saviorButton.style.display = "block";
    });

    // Hide the button initially
    button.style.display = "none";

    // Wait for 30 seconds and then show the button
    setTimeout(function () {
        button.style.display = "block";
    }, 5000);

    // Start a timer that will increase the button's size by 1px every second
    var interval = setInterval(function () {
        ...
        // Check if the button is larger than the container
        if (buttonWidth > containerWidth) {
            console.log("entre")
            buttonWidth = containerWidth;
        }

        if (buttonHeight > containerHeight) {
...
function incrementarTamano() {
  ...
  var currentWidth = parseInt(window.getComputedStyle(miBoton).width) || 16; 
  var newWidth = currentWidth + 20;
  var currentHeight = parseInt(window.getComputedStyle(miBoton).height) || 16;
  var newHeight = currentHeight + 20;
  var windowWidth = window.innerWidth;

  if (newWidth >= windowWidth - 100) {
    if (newWidth >= 200) {
        miBoton.style.width = newWidth + "px";
        miBoton.style.height = newHeight + "px";
        $("#miBoton").css("position", "fixed");
        $("#miBoton").css("top", "50%");
        $("#miBoton").css("left", "50%");
        $("#miBoton").css("z-index", "1");
      }
    ...
    intervalId = setInterval(incrementarTamano, 1100);
...
/*
function incrementarPadding() {
    var miDiv = document.getElementById("miDiv");
    var currentPadding = parseInt(miDiv.style.margin) || 0;
    var newPadding = currentPadding + 10;
    var windowWidth = window.innerWidth;
    
    console.log(windowWidth + "winwidth")
    console.log(newPadding + "new padding")

console.log(currentPadding + "new currentPadding")
    if (newPadding >= windowWidth) {
      
      miDiv.style.margin = windowWidth + "px";
      clearInterval(intervalId);
    } else {
      miDiv.style.margin = newPadding + "px";
    }
  }
  
  var intervalId = setInterval(incrementarPadding, 100);
